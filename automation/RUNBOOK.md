...
| 필드 | 내용 |
|---|---|
| `date`, `emoji`, `title`, `hero_title`, `subtitle` | 제목은 두 유형 중 하나로 짧게(두 줄, 줄마다 12자 안팎): **질문·반전형**(뉴닉식, `토스가 이번엔, 새벽배송을 품었다고?`) 또는 **쓸모형 목록**(비스킷식, `회의 때 꺼내기 좋은, 추석 마케팅 숫자 6`). `hero_title` 은 뉴스레터 머리 제목(카드 표지 기본값) — 두 줄(`\n`)로 쓰면 첫 줄은 굵게, 둘째 줄은 보통 굵기. `title` 은 빅이슈 제목으로도 쓰인다 |
