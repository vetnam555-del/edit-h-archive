...
| 필드 | 내용 |
|---|---|
| `date`, `emoji`, `title`, `hero_title`, `subtitle` | 제목은 두 유형 중 하나로 짧게(두 줄, 줄마다 12자 안팎): **질문·반전형**(뉴닉식, `토스가 이번엔, 새벽배송을 품었다고?`) 또는 **쓸모형 목록**(비스킷식, `알아두면 좋은, 추석 트렌드 숫자 6`). `hero_title` 은 뉴스레터 머리 제목(카드 표지 기본값) — 두 줄(`\n`)로 쓰면 첫 줄은 굵게, 둘째 줄은 보통 굵기. `title` 은 빅이슈 제목으로도 쓰인다 |
...
| `big_issue` | `tag`, `paragraphs`(2), `stat`(`value`·`unit`·`caption`), `takeaway`, `checklist`(1~3, '알아두면 좋은 것' — 누구에게나 쓸모 있는 것), `sources` |
| `sections` | 2~4개, 아이템 합계 **정확히 9개**(보통 3×3). 섹션: `label`(카테고리, 예 `생활/소비 & 여행/레저`)·`title`. 아이템: `tag`, `title`, `body`(2~3문장), `takeaway`(1문장), `sources` |
| `briefs` | 0~3개 짧은 소식 |
| `question` | `text`(강조 `==…==`, 줄바꿈 `\n`) — 누구나 자기 경험으로 답할 수 있는 질문, `closing` |
| `cards` | 카드뉴스 10장용 — 아래 '카드뉴스' 참고 |
...
### 카드뉴스 (`cards`, 10장 — VOL.092 매거진 엔진 + 개선안 B + 레퍼런스 보강)

`01 표지 → 02 오늘의 6가지 요약 → 03~08 이슈 6장 → 09 H의 한 줄 관찰 → 10 마무리`. 뉴스레터 10개 중 **6개**를 골라 한 장에 한 주제·한 숫자로 만든다.
요약 카드는 이슈 카드의 제목·숫자로, 관찰 카드는 `observation`(90자 이하, 결론은 ` — ` 뒤에 — 형광 마커가 칠해진다)으로 자동으로 만들어진다.

| 필드 | 내용 |
|---|---|
| `cards.cover` | `title`(두 줄 — 각 줄이 띠 상자로 나간다, 없으면 `hero_title`), `keyword`(사진이 없을 때 크게 찍힐 핵심어 5자 이하 — 없으면 `hero_title` 의 `==강조==`). 표지 스티커 3개는 2~4번 이슈의 태그·숫자로 자동 생성. 사진을 쓰려면 `photo`(저장소 안 경로)·`credit`(출처·라이선스)·`focus`(선택, CSS background-position 예 `center 40%`). 사진 표지는 사진을 끝까지 보여주고 위·아래만 어둡게 받치며 스티커는 빠진다. 없으면 흰 핵심어 표지 |
| `cards.issues` | 정확히 6개. `no`(뉴스레터 이슈 번호 1~10 — 태그·출처를 거기서 가져온다), `number`(큰 숫자, 9자 이하 예 `300억원`·`82%`), `headline`(두 줄 26자 이하, 핵심어 하나 `==강조==`), `body`(**1~2문장, 80자 이하** — 뉴스레터 본문을 그대로 옮기지 말고 핵심만), `takeaway`(60자 이하 — '그래서 뭐가 달라져?'에 대한 답 말풍선, 누구나 읽을 수 있게). 선택: `metrics`(큰 숫자를 받쳐줄 보조 수치 `[{"value": "11.9%", "label": "8월 백화점 매출 증가"}]` 최대 2개, 값 8자·설명 16자 이하), `accent: true`(H PICK — 검정 반전, **하루 한 장**), `compare`(`{"from": {"value", "label"}, "to": {...}}` — 이전→이후 숫자가 핵심일 때, 이때 `number` 는 생략), `tag`(말풍선 문구를 바꿀 때) |
