...
스티커 3개, 인사이트를 '그래서 마케터는?' 대화 말풍선으로, 캡션을 '질문형 훅 → 오늘의 목록 → 저장·댓글 유도 → 댓글 키워드 DM 안내'로.
표지 실사 사진은 위키미디어 커먼즈의 CC0·퍼블릭 도메인·CC BY 사진만(`automation/fetch_photo.py`, 네트워크 허용 필요) 쓰고,
못 구하면 핵심어 표지로 자동 전환한다. 비스킷(@biscit.co.kr) 비교 후: 표지 상단 `EDIT H.` 워드마크(모든 표지 같은 자리 — 공유·저장돼도 브랜드가 보이게), 쓸모형 목록 제목 유형, 캡션에 표지 사진 출처와 브랜드 서명 줄. 댓글 키워드는 `config.json` 의 `instagram.dm_keyword`(기본 '에디트') — DM 은 직접 보낸다.
