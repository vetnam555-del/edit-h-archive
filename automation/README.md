...
07:00 KST  Claude 클라우드 루틴 시작 ─ automation/RUNBOOK.md 대로
           ├ 안장출근길 확인 → 없으면 주요 매체 검색으로 메인 5가지(H PICK 심층 1 + 4) + 한 줄 뉴스 5개 선정·사실 확인
...
## 예비 호 — 제작 루틴이 멈춘 날 (2026-09-26~)

- `rewind.yml`(08:07·08:37·09:07·09:47 KST): 발행일인데 08:05 까지 오늘 호가 없으면 `build_rewind.py` 가 최근 3주 호에서
  이야기 5개(형식 2 H PICK 1 + 카드가 있던 이야기 4, 태그가 겹치지 않게, 성과표 점수·최신 순, '오늘·내일' 같은 시점 표현이 든 것은 되도록 뺌)를
  다시 엮어 '놓쳤다면, 다시 볼 만한 5가지'를 만들고, 빌드 → main 푸시 → 발송·인스타 게시 실행 → 운영 알림 메일까지 한다(Claude 없이 GitHub 만으로).
- 새로 취재하지 않으니 틀린 사실이 나갈 위험이 가장 낮다. 출처 옆 날짜가 처음 실린 날이고, 에디터 H 노트가 '다시 보기'임을 밝힌다.
- 시험: Actions → EDIT H reserve issue → Run workflow(`dry_run` 켠 채, 날짜 비우거나 넣기) — 빌드까지만 하고 아무것도 올리지 않는다.
...
## 형식 2 — 메인 5가지 + 한 줄 뉴스 5개 + H PICK 심층 + 에디터 H 노트 + 주간 투표 (2026-09-26~)

2026-09-25 경쟁 매체 비교(뉴닉·어피티·캐릿·순살브리핑) 뒤 '많이가 아니라 깊게'로 바꿨고, 2026-09-26 사용자 결정(절충안)으로
한 줄 뉴스를 5개로 늘려 하루 10개 주제를 다시 다룬다(메인 5가지는 깊게, 한 줄 뉴스 5개는 짧게). 읽는 시간 약 5분.
- **뉴스레터**: 에디터 H 노트(안경 마크 · 1인칭 2~3문장 · 오늘의 5가지 · H의 한 줄) → 01 H PICK 심층(무슨 일이야 → 큰 숫자 →
  숫자로 보면 → 왜 중요해 → 그래서 뭐가 달라져 → 알아두면 좋은 것) → 02~05 → 한 줄 뉴스 06~10(5개, 부족한 날 3개~) → (금) 투표 결과 → Q 또는 (월) 투표.
  노트의 '오늘의 5가지' 아래에 '+ 한 줄 뉴스 N개(06–10)' 안내가 붙는다. 한 줄 뉴스는 메일 전용(카드뉴스는 메인 5가지만).
